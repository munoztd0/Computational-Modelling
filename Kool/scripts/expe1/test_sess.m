    ...
    for k_sim = 1:models
        ...
        for k_sub=1:nsub
            for k_sess=1:sess
                ...
                SimRun(k_it).simu_param(k_sub, k_sess, k_sim,:)  = [B1(k_sub), B2(k_sub), B3(k_sub), LR(k_sub), LAMBDA(k_sub), W1(k_sub),W2(k_sub),W3(k_sub)].*KK(k_sim,:);
                ...
                %% SARS
                output  = C_ii_MBMF_sim(squeeze(SimRun(k_it).simu_param(k_sub, k_sess, k_sim,:)),con, rews, k_sim);   %# returns S A R & S1
                ...
                for k_est=1:models
                    ...
                    SimRun(k_it).recov_param(k_sim).val(k_sub,k_sess,k_est,:)  = parameters_rep.*KK(k_est,:);
                    SimRun(k_it).ll(k_sim).val(k_sub,k_sess,k_est)             = ll_rep;
...
%             
%             for i = 1:sess
%                 x2 = SimRun(k_it).mean(i).recov_param.val(k_sub,k_est,:) + x;
%                 x = SimRun(k_it).mean(i).recov_param.val(k_sub,k_est,:); 
%                     
%                 y2 = SimRun(k_it).mean(i).ll.val(k_sub,k_est,:) + y;
%                 y = SimRun(k_it).mean(i).ll.val(k_sub,k_est,:);  
%             end
%                 
%             SimRun(k_it).recov_param(k_sim).val(k_sub,k_est,:)  = x2/sess;
%             SimRun(k_it).ll(k_sim).val(k_sub,k_est,:)            = y2/sess;
...
save(['SIMU_RECOVERY_Kool_sess' n_iter],'SimRun')
