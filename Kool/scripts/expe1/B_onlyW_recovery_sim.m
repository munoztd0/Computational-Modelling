...
%notes
KK = [1 0 0;... 4 - 1w 
      0 1 1;... 6 - 2w  / stage0 at median 
      1 0 1;... 7 - 2w  /  stage1 at median 
      1 1 1];%. 8 - 3w  
...
for k_it = 1:iterations %# 50 before
    ...
    close all force
        ...
        B2 = median(params.model_exhaustive(:,2));
        B3 = median(params.model_exhaustive(:,3));
        ...
        LR = median(params.model_exhaustive(:,4));
        ...
        W0  = median(params.model_exhaustive(:,6));
    ...
    for k_sim = 1:models
        ...
        for k_sub=1:nsub
            ...
            %% SARS
            output  = C_ii_onlyW_sim(squeeze(SimRun(k_it).simu_param(k_sub,k_sim,:)),con, rews, k_sim, B1, B2, B3, LR, LAMBDA, W0);   %# returns S A R & S1
            SimRun(k_it).rewardrate(k_sim).val(k_sub) = sum(output.R)/length(output.R); 
            % re-estimate parameters with all possible models
            
            for k_est=1:models 
                x0                                                  = [rand() rand() rand()];% parameter initial value ?ok at ten?            
                [parameters_rep(1,1:param),ll_rep]                  = fmincon(@(x) C_iii_onlyW_model_ll(x,con, output, k_est, B1, B2, B3, LR, LAMBDA, W0),x0,[],[],[],[],LB,UB,[],options); %changed out by rews
