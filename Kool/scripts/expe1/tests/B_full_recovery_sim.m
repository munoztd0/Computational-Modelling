...
KK = [1 1 0 0 0;... %1 MF
      1 0 0 0 0;... %2 MB
      1 1 0 1 0;... %3 Mix 2 w
      1 1 0 1 1];%... %4Always have a full model
...
for k_it = 1:iterations %# 50 before
    ...
    for k_sim = 1:models
        ...
        for k_sub=1:nsub
            ...
            %% SARS
            output  = C_ii_full_sim(squeeze(SimRun(k_it).simu_param(k_sub,k_sim,:)),con, rews, k_sim);   %# returns S A R & S1
            ...
            for k_est=1:models
                ...
                [parameters_rep(1,1:param),ll_rep]                  = fmincon(@(x) C_iii_full_model_ll(x,con, output, k_est),x0,[],[],[],[],LB,UB,[],options); %changed out by rews
...
save('SIMU_RECOVERY_Kool_BFWW_30x20','SimRun')
