...
KK = [1 1 0 0 0;... %1 MF
      1 1 0 1 0];%... %2 Mix W
      %1 1 1 0 1 1;... %3 Mix 2 w
      %1 1 1 1 1 1];%... %4Always have a full model
...
%nfpm = [3 3 5 5 4 6 6 8]; 
%nfpm = [4 4 6 6 4 6 6 8];
...
for k_it = 1:iterations %# 50 before
    ...
    for k_sim = 1:models
        ...
        for k_sub=1:nsub
            ...
            SimRun(k_it).simu_param(k_sub,k_sim,:)  = [B1(k_sub), LR(k_sub), LAMBDA(k_sub), W1(k_sub), W2(k_sub)].*KK(k_sim,:); %
            ...
            %% SARS
            output  = C_ii_Mix_sim(squeeze(SimRun(k_it).simu_param(k_sub,k_sim,:)),con, rews, k_sim);   %# returns S A R & S1
            SimRun(k_it).rewardrate(k_sim).val(k_sub) = sum(output.R)/length(output.R); 
            % re-estimate parameters with all possible models
            for k_est=1:models
                  
                x0                                                  = [5*rand() rand() rand() 0.5 0.5];% rand() rand()]; % 10*rand() 10*rand()% parameter initial value ?ok at ten?
                [parameters_rep(1,1:param),ll_rep]                  = fmincon(@(x) C_iii_mix_model_ll(x,con, output, k_est),x0,[],[],[],[],LB,UB,[],options); %changed out by rews
...
save('SIMU_RECOVERY_Kool_MBvW_30_same_param.mat','SimRun')
