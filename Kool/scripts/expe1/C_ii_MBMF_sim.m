...
%              %notes
            ...
            %high_Q(2,:) = w*Qmb2(s(2),:) + (1-w)*Qmf2(s(2),:);
            high_Q(2,:) = Qmf2(s(2),:);
            ps = exp(b3*high_Q(2,:))/sum(exp(b3*high_Q(2,:)));                      %compute choice probabilities for each action
            a(2) = find(rand<cumsum(ps),1);                   % choose
    ...
    if model == 7
        ...
        if c(k) == 1 
            ...
            %% choices + updating
            % level 0
            high_Q(1,:) = w2*Qmb1' + (1-w2)*Qmf1(s1_stims); 
            ps = exp(b1*high_Q(1,:))/sum(exp(b1*high_Q(1,:)));                      %compute choice probabilities for each action
            action = find(rand<cumsum(ps),1);                 % choose
            ...
            high_Q(2,:) = w3*Qmb2(s(2),:) + (1-w3)*Qmf2(s(2),:);
            ps = exp(b1*high_Q(2,:))/sum(exp(b1*high_Q(2,:)));                      %compute choice probabilities for each action
            a(2) = find(rand<cumsum(ps),1);                   % choose
       ...
       %low effort
            ...
            low_Q = w1*Qmb + (1-w1)*Qmf(s1,:)';                 % mix TD and model value
