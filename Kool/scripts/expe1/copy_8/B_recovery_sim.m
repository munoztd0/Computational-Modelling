...
UB = [10 10 10 1 1 1 1 1]; 

%notes
%simu.recov -> column = model // val = param
% Model options #well .. model 6 first for now
KK = [1 0 0 1 1 0 0 0;... %1 MF simple %chanef 6 =0
      1 0 0 1 1 0 0 0;... %2 MB simple %chanef 6 =0
    ...
    for k_sim = 1:models
        ...
        for k_sub=1:nsub
            ...
            for k_est=1:models
                ...
                x0                                                  = [10*rand() 10*rand() 10*rand() rand() rand() rand() rand() rand()]; % parameter initial value ?ok at ten?
...
save('SIMU_RECOVERY_Kool_mac_test_no_param','SimRun')
