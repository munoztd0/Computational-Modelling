...
KK = [1 1 1 1;... %1 MF simple %chanef 6 =0
      1 1 1 1]; %... %2 MB simple %chanef 6 =0
...
for k_it = 1:iterations %# 50 before
    ...
    for k_sim = 1:models
        ...
        for k_sub=1:nsub
            ...
            SimRun(k_it).simu_param(k_sub,k_sim,:)  = [B1(k_sub), LR(k_sub), LAMBDA(k_sub), W1(k_sub)].*KK(k_sim,:); %,W2(k_sub),W3(k_sub)
...
save('SIMU_RECOVERY_Kool_new_20f1','SimRun')
