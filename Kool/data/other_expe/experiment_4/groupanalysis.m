...
for i = 1:nrsubs
    ...
    if mean(subdata.missed) < 0.2 % throw out participants with more than >20% trials missed
        ...
        groupdata.rewardrate(s,1) = mean(subdata.win(~subdata.missed&~subdata.probe_trial)) - mean(mean(subdata.probs(~subdata.probe_trial,:))); % average reward rate (corrected by chance performance)
