	...
	jsPsych["2step-complexity-stim"] = (function() {
		...
		plugin.create = function(params) {
			...
			for (var i = 0; i < trials.length; i++) {
				...
				trials[i].feedback_time = params.feedback_time || 1000; //500
		...
		plugin.trial = function(display_element, trial) {
			...
			if (trial.probe_trial==0) {
				...
				var actual_probs = trial.probs;
				var high_effort = Math.round(Math.random());
				
			} else {
				
				var actual_probs = [0,0,0];
			...
			var end_trial = function(){
				...
				var trial_data = {
					...
					"probs1": trial.probs[0],
					"probs2": trial.probs[1],
					"probs3": trial.probs[2],
					"actual_probs1": actual_probs[0],
					"actual_probs2": actual_probs[1],
					"actual_probs3": actual_probs[2],
			...
			var after_response = function(info){
				...
				if (pause == 0) {
					...
					if (part < 2) { // space stations or rockets
						...
						win = Math.random()<actual_probs[current_state-1];
						
						points = win*trial.probe_reward;
				...
				if (stage==3) { // reward
					...
					if (win==0) {
						...
						if (points!=trial.probe_reward) {
							if (show_points==0) {
								$('#jspsych-2step-complexity-bottom-stim-middle').css('background-image', 'url(img/'+state_name+'_stim.png)');
								$('#jspsych-2step-complexity-top-stim-middle').css('background-image', 'url(img/treasure.png)');
							} else {
								$('#jspsych-2step-complexity-bottom-stim-middle').css('background-image', 'url(img/'+state_name+'_stim.png)');
								$('#jspsych-2step-complexity-top-stim-middle').css('background-image', '');
								$('#jspsych-2step-complexity-top-stim-middle').html('<br><br>+1');
							}
							if (trial.practice == 0) {
