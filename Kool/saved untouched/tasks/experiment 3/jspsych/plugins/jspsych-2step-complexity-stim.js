	...
	jsPsych["2step-complexity-stim"] = (function() {
		...
		plugin.create = function(params) {
			...
			for (var i = 0; i < trials.length; i++) {
				...
				trials[i].feedback_time = params.feedback_time || 0; //500
		...
		plugin.trial = function(display_element, trial) {
			...
			if (high_effort==1) {
				...
				var extra_fb_time = (trial.timing_response + trial.feedback_time)/2;
			...
			var after_response = function(info){
				...
				if (pause == 0) {
					...
					if (trial.timing_response>0) {
						var extra_time = trial.timing_response-response[part].rt+extra_fb_time;
					} else {
						var extra_time = 500;
					}
					...
					if (part < 2) { // space stations or rockets
						...
						win = Math.random()<trial.probs[current_state-1];
