...
for s = 1:nrsubs
    ...
    trials = cell2mat(data(strcmp(cellstr(data(:,1)),cellstr(subinfo(s,1))),2:34));
    firsttrial  = trials(1,:);
    ...
    % only in probes condition
    subdata.probe_trial = trials(:,25);
    subdata.probe_index = trials(:,26);
    subdata.probe_effort = trials(:,27);
    subdata.probe_target = trials(:,28);
    subdata.probe_starting_state = trials(:,29);
    subdata.actual_probs1 = trials(:,30);
    subdata.actual_probs2 = trials(:,31);
    subdata.actual_probs3 = trials(:,32);
    subdata.actual_probs = trials(:,30:32);
    subdata.probes_correct = trials(:,33);
